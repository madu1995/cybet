...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        // සර්වර් එකත් එක්ක සොකට් කනෙක්ෂන් එක හදාගැනීම
        ...
        socket.on('leaderboard_update', (data) => {
            ...
            if (myInfo) {
                ...
                setCurrentUser(prev => prev ? { ...prev, rank: myInfo.rank } : null);
            }
        });
        }
        ...
        // ලයිව් ටයිමර් එක ඇල්ලීම
        if(socket){
        socket.on('timer', (data) => {
            setTimer(data.timer);
            
            // අලුත් රවුන්ඩ් එකක් ස්ටාර්ට් වෙද්දීම මැද තියෙන පරණ රිසල්ට් එක ක්ලියර් කරනවා
            if (data.timer === 29) {
                setIsRolling(false);
        ...
        // ටයිමර් එක 0 වුණ ගමන් සර්වර් එකෙන් ලයිව් රිසල්ට් එක එනවා
        if(socket){
        socket.on('game_result', (data) => {
            setIsRolling(true);
            setDiceResult(null); 

            // ඇනිමේෂන් එක තත්පර 5ක් ප්ලේ වෙන්න දෙනවා
            setTimeout(() => {
                setIsRolling(false);
        ...
        // බෙට් කළත් නැතත් හැමෝගෙම පොදු හිස්ට්‍රි එක ලස්සනට අප්ඩේට් කරන ඉවෙන්ට් එක
        if(socket){
        ...
        socket.on(`round_result_${user._id}`, (data) => {
            // අංකය වැටිලා මිලි තත්පර 500කින් මැසේජ් එක පෙන්වීම
            setTimeout(() => {
                ...
                if (data.isWin) {
                    ...
                    setGameAlert({
                        show: true, // පැරදුණත් මැසේජ් එක පෙන්වීමට true කළා
                        isWin: false,
            ...
            // මැසේජ් එක තත්පර 3ක් පෙනී තිබී අයින් කිරීම
            setTimeout(() => {
                setGameAlert({ show: false, isWin: false, message: '' });
            }, 8500);
        });
    ...
    // පේජ් එක ලෝඩ් වෙද්දී සහ ලයිව් අප්ඩේට් වෙද්දී හිස්ට්‍රි එක රන් වීම
    useEffect(() => {
        fetchBetHistory();
    ...
    const handlePlaceBet = (e) => {
        ...
        if (!currentUser || currentUser.balance < Number(betAmount)) {
            setBetMessage({ type: 'error', text: 'ඔට්ටුව තැබීමට ප්‍රමාණවත් මුදලක් නොමැත!' });
            return;
        }

        // සර්වර් එකට ඩේටා යැවීම
        if (socket) {
        ...
        // බෙට් එක දාපු සැනින් උඩ ප්‍රොෆයිල් බැලන්ස් එක තාවකාලිකව අඩු කිරීම
    ...
    // [පට්ටම වැදගත්]: රිෆ්‍රෙෂ් කරද්දී යූසර් ඩේටා null නම් ඇප් එක ක්‍රෑෂ් නොවී හෝල්ඩ් කරගැනීම
    if (!user || !currentUser) {
        return (
            <div className="min-h-screen bg-cyberBg flex items-center justify-center text-cyan-400 font-mono">
                <div className="animate-pulse">Loading Cyber Dashboard...</div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-cyberBg text-white font-sans">
                ...
                <div className="flex items-center space-x-6">
                    <div className="bg-black/40 border border-cyberBlue/20 rounded-xl px-4 py-2 flex items-center space-x-3 shadow-[0_0_15px_rgba(0,210,255,0.05)]">
                        <div className="text-right">
                            ...
                            <p className="text-sm font-bold text-green-400">Rs. {currentUser?.balance?.toLocaleString() || '0'}.00</p>
                        </div>
                    ...
                    <div className="flex items-center space-x-3 border-l border-gray-800 pl-6">
                        <div className="text-right">
                            <p className="text-xs font-bold text-gray-300">{currentUser?.name || 'Loading...'}</p>
                            <p className="text-[10px] text-cyberBlue uppercase tracking-wider">{currentUser?.role || 'User'}</p>
                        </div>
            ...
            {/* 🎮 මැද ප්‍රධාන සෙල්ලම් කරන ඒරියා එක */}
            <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                
                <div className="md:col-span-2 flex flex-col gap-4">
                    {/* වම්පැත්තේ කොටස: Live Dice Mat */}
                    <div className="bg-cyberDark border border-cyberBlue/10 rounded-2xl p-8 flex flex-col items-center justify-center relative overflow-hidden shadow-[0_0_30px_rgba(0,210,255,0.02)] h-[400px]">
                        <div className="absolute top-4 left-4 right-4 flex justify-between items-center">
                            ...
                            {/* 📊 මෑතකදී වැටුණු අංක (Recent Rounds UI) */}
                        ...
                        {/* 🔥 ලයිව් දිනුම්/පැරදුම් ඇලර්ට් එක */}
                        {gameAlert.show && (
                    ...
                    {/* 📊 ඔට්ටු ඉතිහාසය සහ සංඛ්‍යාලේඛන පැනලය */}
                    <div className="bg-black/40 border border-cyberBlue/10 rounded-2xl p-5 shadow-[0_0_15px_rgba(0,210,255,0.02)] flex flex-col h-[320px] w-full">
                        ...
                        {/* ක්ෂණික සංක්ෂිප්තය (Quick Stats Summary) */}
                        <div className="grid grid-cols-3 gap-2 mb-4">
                            ...
                            <div className="bg-black/20 border border-gray-850 p-2 rounded-lg text-center">
                                ...
                                <p className={`text-sm font-bold font-mono mt-0.5 ${betSummary.isPositivePNL ? 'text-green-400' : 'text-red-400'}`}>
                                    {betSummary.netPNL}
                                </p>
                            </div>
                        </div>

                        {/* විස්තරාත්මක හිස්ට්‍රි ටේබල් එක */}
                                    ...
                                    <tbody className="divide-y divide-gray-850/50 font-medium text-gray-300">
                                        {betHistory.map((bet) => (
                                            <tr key={bet._id}>
                                                ...
                                                <td className={`py-2.5 text-right font-mono ${bet.status === 'WIN' ? 'text-green-400' : 'text-gray-500'}`}>
                ...
                {/* දකුණුපැත්තේ කොටස: REAL-TIME BETTING PANEL */}
                <div className="bg-cyberDark border border-cyberBlue/10 rounded-2xl p-6 flex flex-col justify-between">
                    <div>
                        ...
                        <form onSubmit={handlePlaceBet} className="space-y-4">
                            ...
                            <button 
                                type="submit" 
                                disabled={timer <= 5 || !!myCurrentBet || !currentUser || currentUser?.balance <= 0}
                                className={`w-full font-bold py-3 rounded-xl transition-all ${
                                    timer <= 5 || !!myCurrentBet || !currentUser || currentUser?.balance <= 0
                    ...
                    <div className="my-5 bg-black/40 border border-cyberBlue/10 rounded-xl p-4 shadow-[0_0_15px_rgba(0,210,255,0.02)] flex flex-col max-h-[290px]">
                        ...
                        {/* 🎯 ලොග් වෙලා ඉන්න කෙනාගේ Rank එක 10ට වඩා වැඩි නම් පල්ලෙහා ස්ටිකි බාර් එක පෙන්වීම */}
                        {currentUser?.rank > 10 && (
                            <div className="mt-3 pt-2 border-t border-dashed border-gray-800 flex justify-between items-center bg-gradient-to-r from-purple-950/40 to-black/20 p-2 rounded-lg border border-purple-500/20">
                                ...
                                <span className="font-mono text-green-400 text-[11px] font-bold">
